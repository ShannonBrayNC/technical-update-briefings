...
def add_title_box(slide, text, *, left_in, top_in, width_in, height_in, font_size_pt=60, bold=True, color="#FFFFFF", align=PP_ALIGN.LEFT):
    ...
    r.font.bold = bold
    r.font.color.rgb = RGBColor.from_string(_normalize_hex(color))
...
def add_cover_slide(prs, assets, cover_title, cover_dates, logo1_path, logo2_path):
    ...
    add_title_box(
        slide, cover_title or "Title",
        left_in=0.5, top_in=2, width_in=9, height_in=2,
        font_size_pt=52, color="#FFFFFF", align=PP_ALIGN.CENTER
    )
    if cover_dates:
        add_text_box(
            slide, cover_dates,
            left_in=0.5, top_in=4, width_in=9, height_in=1,
            font_size_pt=30, color="#FFFFFF", align=PP_ALIGN.CENTER
        )
    if logo1_path:
...
def add_agenda_slide(prs, assets, agenda_lines=None):
    ...
    add_title_box(
        slide, "Agenda",
        left_in=0.5, top_in=0.9, width_in=9, height_in=1.2,
        font_size_pt=52, color="#FFFFFF", align=PP_ALIGN.LEFT
    )
    if not agenda_lines:
    ...
    for line in agenda_lines:
        add_text_box(
            slide, f"• {line}",
            left_in=0.5, top_in=top, width_in=9, height_in=0.5,
            font_size_pt=26, color="#FFFFFF", align=PP_ALIGN.LEFT
        )
        top += 0.6
...
def add_separator_slide(prs, assets, title):
    ...
    add_title_box(
        slide, title,
        left_in=0.5, top_in=3.2, width_in=9, height_in=1.5,
        font_size_pt=56, color="#FFFFFF", align=PP_ALIGN.CENTER,
    )
...
def add_conclusion_slide(prs, assets, links):
    ...
    for text, url in links:
        add_text_box(
            slide, f"{text}: {url}",
            left_in=0.5, top_in=top, width_in=9, height_in=0.5,
            font_size_pt=22, color="#FFFFFF", align=PP_ALIGN.LEFT,
        )
        top += 0.6
